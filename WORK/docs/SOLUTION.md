...
- Implemented in `WORK/etl/src/mastr_etl/parse.py`.
...
- Each entity type (solar, wind, stromspeicher, …) is defined in
  `WORK/etl/src/mastr_etl/config.py:ENTITIES`, with:
...
- `WORK/etl/src/mastr_etl/build_duckdb.py` creates `mastr.duckdb` from the
...
- `WORK/etl/src/mastr_etl/publish.py` creates a GitHub Release tagged
...
- `WORK/etl/src/mastr_etl/schema_diff.py` compares today's parquet column sets
...
- This is the only step that requires human intervention. Typical response
  ...
  (3) mirror the change in `WORK/shiny/R/mastr_data.R::.create_units_view()` so
  the R-side view stays in sync.

---

## 4. The R-side data layer (`WORK/shiny/R/mastr_data.R`)
...
## 5. The visual layer (`WORK/shiny/R/ui_helpers.R`)
...
Each app lives in its own folder `WORK/shiny/apps/NN_name/` with an `app.R`
...
```
mastr-shiny/
├── README.md                  # monorepo landing (WORK + THESIS)
├── Makefile                   # local ETL targets (paths under WORK/)
├── WORK/
│   ├── README.md
│   ├── etl/                   # Python pipeline (mastr_etl)
│   │   ├── pyproject.toml
│   │   ├── schema_snapshot.json
│   │   ├── src/mastr_etl/     # download, parse, duckdb, aggregates, publish, schema_diff
│   │   └── tests/             # pytest smoke tests
│   ├── shiny/                 # production Shiny launcher + apps (Candida / flagship = apps/most_visited)
│   │   ├── app.R
│   │   ├── R/                 # mastr_data.R, ui_helpers.R, …
│   │   └── apps/              # one folder per dashboard
│   └── docs/                  # RUN, AGENT_HANDOFF, RSTUDIO_CONTEXTS, SOLUTION, …
├── THESIS/
│   ├── README.md
│   └── thesis_energy_mastr_shiny/   # thesis launcher + battery / lithium apps
...
|------------------------------------------------|-------------------------------------------------------------------------------------|
| …ingest a new XML entity                       | `WORK/etl/src/mastr_etl/config.py` + mirror in `WORK/shiny/R/mastr_data.R::.create_units_view` |
| …add a new dashboard                           | Copy any `WORK/shiny/apps/NN_…/` → rename → edit title + queries                         |
| …change the palette / fonts                    | `WORK/shiny/R/ui_helpers.R::MASTR_PALETTE` + `mastr_theme()`                              |
...
| …add a new pre-rolled aggregate                | `WORK/etl/src/mastr_etl/aggregates.py` + list in `.create_aggregate_views()` (R)         |
